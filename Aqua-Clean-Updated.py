...
def validate_api_token(api_token):
    test_url = f'{AQUA_API_URL}/api/v1/repositories'
    try:
        response = requests.get(test_url, headers=get_headers(api_token), timeout=10)
...
def get_repository_info(repo_name, api_token):
    try:
        url = f'{AQUA_API_URL}/api/v1/repositories'
        params = {'name': repo_name}
        response = requests.get(url, headers=get_headers(api_token), params=params, timeout=10)
...
def delete_repositories(repo_ids, api_token):
    ...
    try:
        response = requests.delete(url, json=body, headers=get_headers(api_token), timeout=10)
...
if repo_ids_to_delete:
    batch_size = 100
    for i in range(0, len(repo_ids_to_delete), batch_size):
